...
1. **Use the historical version only as a capability reference.** As a maintainer, I want the rewrite to reconsider capabilities from the minimum so obsolete shell interfaces and implementation choices are not preserved accidentally.
   ...
   - After the complete Python collection and cleanup path is implemented, V1 makes a hard cutover before whole-system review and qualification: remove the historical shell collector and verifier, public shell entry points and compatibility wrappers, historical shell-only implementation tests and fixtures, and active build or documentation references.
   - Do not keep a wrapper or `legacy/` source copy; rollback uses Git history, tags, or a previously published release. Preserve only behavior coverage required by this specification in the Python suite.
...
15. Post-cutover negative tests assert there is no tracked, installable, or executable historical shell collector, verifier, public shell entry point, compatibility wrapper, shell-only implementation fixture, or stale active instruction; unrelated maintainer shell tooling is allowed but is not a product path. They also assert there is no `sudo`, `cephadm`, alternate interpreter, package installation, remote kubectl, `kubectl exec`, curl, arbitrary command, redactor, secret scanner, or concurrency surface.
