...
宣告通過前的第二輪雙軸 review 又找出四項並已修正：candidate 的 `errors.log` 不記失敗 node 的 exit code（reference 記 `collector exited <code>`，normalizer 原本把兩邊的整行都壓成同一個 class，於是 node exit code 完全脫離比對；candidate 現在記 reference 同一個數字——remote／transport 的原碼，但 timeout 與「remote 成功卻交出不可用 archive」兩種情況記 2）；`/var/log` 不是目錄時 `logs/var-log/SKIPPED.txt` 被索引成 `exit_code` 0，違反 ADR 0010「marker 代表的證據不完整」；`/var/log` 不存在時 candidate 因 payload accounting 缺失而 fail closed 成 partial，reference 在同一狀態下不收 journal 也不算 partial；ledger 的 C9／C12 指向的測試只斷言 collector 自己固定的 0600，沒有真的驗證「保留原檔權限」（C21 與 B4–B8 同輪補強，見 #42）。同一輪也收緊了 normalizer 本身：node 失敗事件改為連 exit code 一起比對、`timed out`／`incomplete` 兩個過寬的 skip class 收斂到事件本身、`redactions.log` 同一路徑的多筆結果不再被覆蓋、`environment.txt` 不含 `=` 的行不再被靜靜丟掉，並補記兩條原本只寫在程式註解裡的忽略規則（node bootstrap payload、node request ledger 的實作專屬欄位）。

Gate 的覆蓋邊界要誠實記錄：fake `ssh` 站在 SSH 邊界，因此 differential 比較的是工作機端契約；node collector 自身的 evidence surface 不在 differential run 內（兩邊的 node payload 由定義就不同，共用同一份 canned Node Evidence Archive），它的等價性由 ledger 的 N 系列黑箱測試負責。
...
6. #18 差異比對規則：shell 記錄的每一筆 entry，Python 必須有對應且 command 語意相同；Python 額外的 entry 與 SKIPPED artifact 僅限已列舉類別（複製類 evidence、`/var/log` 產出樹、下方第 7 項補列的兩類）。不得整份排除 manifest，否則 N9、N10 這類 command-policy 斷言會失去對照組。
...
7. **移植期間補列的兩類（由 #36 提出，等 #8 確認）。** 兩者都是第 1 項「manifest 是 archive 內全部 evidence 的索引」的機械後果，不是新的取捨，但既然第 6 項是逐類列舉，就必須明寫：
   ...
   - **`/var/lib/ceph` listing 的 `command` 記 `collect-node list <目錄絕對路徑>`**，理由同第 2 項：實際掃描是 `find` 或 `sudo -n find`，隨 EUID 與 sudo 可用性改變，不是穩定契約。代價是 manifest 不再顯示這條掃描是否 privileged，因此改由 `NodeEvidenceSurfaceTests.test_privileged_reads_go_through_noninteractive_sudo` 的 argv ledger 斷言 `sudo -n find` 守住 command policy。另有一個繫在暫時 content safety 上的附帶理由，記於 ADR 0010 的括號註腳；該層由 #44 移除後註腳失效，本條不受影響。

### `--skip-logs` 的 marker 算不算完整證據（由 #18 提出，等 #8 裁定）

`/var/log` 不存在時，`logs/var-log/SKIPPED.txt` 依第 7 項第一小點記 `exit_code` 2：
來源不存在，marker 代表的證據不完整。`--skip-logs` 產生的是同一個路徑的 marker，
但成因不同——是操作者主動不收，不是收不到。目前**維持 `exit_code` 0**（`ceph_incident_node.py`
的 `--skip-logs` 分支），等 #8 在兩種讀法之間裁定：

1. `exit_code` 只回答「這份證據完整嗎」，因此操作者主動停用也記非 0；或
2. 操作者明示不收的證據不算缺漏，marker 本身完整即記 0（現況）。

這一項不影響 #18 的 offline gate 判定：node manifest 由 differential run 的
共用 canned archive 產生，兩邊逐位元組相同，不在被比較的差異面內。
