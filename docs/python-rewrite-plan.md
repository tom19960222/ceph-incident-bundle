...
宣告通過前的第二輪雙軸 review 又找出四項並已修正：candidate 的 `errors.log` 不記失敗 node 的 exit code（reference 記 `collector exited <code>`，normalizer 原本把兩邊的整行都壓成同一個 class，於是 node exit code 完全脫離比對）；`/var/log` 不是目錄或 `--skip-logs` 時，`logs/var-log/SKIPPED.txt` 被索引成 `exit_code` 0，違反 ADR 0010「marker 代表的證據不完整」；`/var/log` 不存在時 candidate 因 payload accounting 缺失而 fail closed 成 partial，reference 在同一狀態下不收 journal 也不算 partial；ledger 的 C9／C12 指向的測試只斷言 collector 自己固定的 0600，沒有真的驗證「保留原檔權限」。同一輪也收緊了 normalizer 本身：node 失敗事件改為連 exit code 一起比對、`timed out`／`incomplete` 兩個過寬的 skip class 收斂到事件本身、`redactions.log` 同一路徑的多筆結果不再被覆蓋、`environment.txt` 不含 `=` 的行不再被靜靜丟掉，並補記兩條原本只寫在程式註解裡的忽略規則（node bootstrap payload、node request ledger 的實作專屬欄位）。
...
- **#18 的 offline observable-contract equivalence gate 已宣告通過（2026-07-30）。** 138 個 inventory 情境全部結案（128 ported、10 shell 實作細節）、13 個 differential scenarios 在同一個 fake world 雙跑、normalizer 的每條忽略規則都在 `docs/differential-normalizer.md` 有據可查，`make validate` 離線可重複全綠。宣告的範圍就是它證明的範圍：**工作機端**的 observable contract；node collector 自身的 evidence surface 由 ledger 的 N 系列黑箱測試（斷言，不是雙跑比對）擔保。#36 已移植 node evidence surface，ledger 不再有 blocked 情境。#19 已建立 Lab Profile、status/discovery/activation workflow 與 strict identity preflight；#20 的 dual-run full-collect gate 尚未完成，因此 real-lab qualification 仍未通過，仍不可宣稱整體 qualification-ready。
...
7. **移植期間補列的類別（由 #36 提出，#18 宣告 gate 通過時由 #8 確認）。** 都是第 1 項「manifest 是 archive 內全部 evidence 的索引」的機械後果，不是新的取捨，但既然第 6 項是逐類列舉，就必須明寫：
   ...
   - **「證據不在」的 marker 也記非 0**：`--skip-logs` 與 `/var/log` 不是目錄這兩種情況，`logs/var-log/SKIPPED.txt` 都記 `exit_code` 2。缺席的原因寫在 marker 內文；`exit_code` 只回答「這份證據完整嗎」，因此操作者主動停用也不例外。兩者都不使 node partial，與 reference 相同。
