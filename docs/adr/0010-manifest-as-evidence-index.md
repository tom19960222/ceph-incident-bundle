...
- 「證據不在」的 marker 一律同一讀法，`/var/log` 這層也不例外：`--skip-logs` 停用收集、`/var/log` 不是目錄，兩者的 `logs/var-log/SKIPPED.txt` 都記 `exit_code` 2。缺席的原因（操作者選擇或路徑不存在）寫在 marker 內文，不寫在 `exit_code` 裡；`exit_code` 只回答「這份證據完整嗎」。兩種情況都不使 node partial——與 reference 一致。
...
- 這兩類是本 ADR 第一條的機械後果，但 #18 的比對規則是逐類列舉，所以 `docs/python-rewrite-plan.md` 的 `## Contract Adjudications` 第 7 項另行補列。該項由 #36 提出，並在 #18 宣告 offline gate 通過時由 #8 確認：沒有新的等價／安全取捨，只是把第一條的後果寫成可比對的清單。
...
- #18 差異比對：shell 記錄的每一筆 entry，Python 必須有對應且 command 語意相同；Python 額外的 entry 與 SKIPPED artifact 僅限已列舉類別（複製類 evidence、`/var/log` 產出樹、未執行指令的 SKIPPED marker、「證據不在」的 marker、`/var/lib/ceph` listing 的 collector verb）。manifest 不得整份排除於比對之外，否則 N9、N10 等 command-policy 斷言會失去對照組。
